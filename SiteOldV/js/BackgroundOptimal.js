	...
	{ // Center FLogo ---------------------------
		...
		var sLogo = new FSprite(sLogo,"img/FLogo.png", 100,100);
	...
	{ // Background Update Stacks ----------------------------
		...
		function OnGameStart()
		{
			...
			for (i = 0; i < 24; i++){ var dot = new Instance_ParticleDot(); particleDots.push(dot); dot.index = i; }
		...
		function GameRender() 
		{
			...
			particleDots.forEach(function(i) 
			{
				...
				while(ni.nearest != null && conn < safe)
				{
					...
					conn+=1;
					ni = ni.nearest;
					if ( conn >= safe ) console.log("No nie");
				}
				
				if ( conn > 2 )
				{
					 context.closePath();
					 context.strokeStyle = rgb(.8,.2,.2, i.lineAlpha / 255 );
					 context.stroke();
					 
					 context.fillStyle = rgb(1,0,0, i.toLogoAlpha * 0.25 * i.alpha / 255);
					 context.fill();
			...
			particleDots.forEach(function(i) 
			{
				...
				grad.addColorStop(0.0, rgb(1,0,0, i.toLogoAlpha * 0.25 * i.alpha / 255) );
			...
			// Draw logo
			logoRounding.Draw(raySourceX, raySourceY, TimeElapsed * 10, 1, 1, 0.35);
			logoInner.Draw(raySourceX, raySourceY, 0, 1, 1, 0.5);
